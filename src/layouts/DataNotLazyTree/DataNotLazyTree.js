...
class  DataNotLazyTree extends Component {
    ...
    componentDidMount() {
        ...
        let etId = this.props.etId;
        /*const id = this.props.match.params.id;
        this.props.apiService.getList(id)
            .then(response => {
        ...
        this.getList(currentNode, filters, sorter, paging, true, etId);
        this.setState({
            activeColumns: this.rebuildColumns(selectedColumns, columnCoef)
        });
    }

    getList(currentNode, filtering, sorting, paging, changingPage, etId) {
        ...
        this.props.apiService.getList(etId)
            .then(response => {
                    if(changingPage) {
                        let newPaging = new Paging();
                        if(response) {
    ...
    onExpand(event) {
        this.setState({
            items: this.transformItems(event.node, event.node.data.children)
        })
...
export default withTranslation()(DataNotLazyTree);